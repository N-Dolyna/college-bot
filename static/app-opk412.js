...
async function loadAssignmentsForCourse(courseId, courseName='') {
  ...
  try {
    ...
    assignments.forEach(hw=>{
      ...
      html += `<div class="homework-card status-${hw.status}"><div class="homework-header"><div class="homework-subject">${courseName||''}</div><div class="homework-status status-${hw.status}">${hw.status}</div></div><div class="homework-title">${hw.title}</div><div style="font-size:13px; color:var(--text-secondary); margin-top:8px;">${hw.description? (hw.description.length>200? hw.description.slice(0,200)+'…': hw.description) : ''}</div><div class="homework-deadline">📅 ${deadline}</div><div style="margin-top:10px; display:flex; gap:8px;">${hw.alternateLink ? `<a href="${hw.alternateLink}" target="_blank" rel="noopener noreferrer" style="text-decoration: none;"><button class="btn btn-secondary">🔗 Відкрити в Classroom</button></a>` : `<button class="btn btn-secondary" disabled style="opacity: 0.5; cursor: not-allowed;">Немає посилання</button>`}</div></div>`;
    });
    container.innerHTML = html;
  } catch(e) {
